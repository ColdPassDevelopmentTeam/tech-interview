...
다시 한 번 말하지만 우리의 코드는 사용하는 부분만 계속 사용합니다. 즉, 모든 메모리 영억을 항상 다 적재해 둘 필요는 없습니다.
...
> **🚨 주의:** 스토리지 접근은 CPU 입장에서 '죄악'에 가깝습니다. Context Switching(마이크로초 단위)이 느리다고 하지만, Disk I/O(밀리초 단위)는 인간이 체감할 정도로 **압도적으로 느립니다.** 따라서 페이지 폴트 비율을 줄이는 것이 성능의 핵심입니다.
...
#### 7. 심화: 알아두면 좋은 (사실 알아야 하는) 것들

**계층적 페이징 (Hierarchical Paging)**
![계층적paging](./src/21.png)
* 32비트 시스템 기준 4KB 페이지를 쓰면 페이지 테이블 항목이 약 100만 개($2^{20}$)가 필요합니다. (약 4MB 크기)
* 프로세스마다 4MB씩 차지하면 낭비가 심하므로, **"페이지 테이블을 위한 페이지 테이블"**을 만들어 필요한 부분만 메모리에 할당하는 방식입니다.

**Copy-on-Write (COW)**
![cow](./src/22.png)
* 프로세스를 복제(`fork()`)할 때 처음부터 물리 메모리를 다 복사하지 않습니다.
* 부모와 자식이 같은 메모리를 공유하다가, **누군가 '쓰기(Write)' 연산을 할 때 비로소 복사**를 수행하여 메모리 낭비와 생성 시간을 줄입니다.